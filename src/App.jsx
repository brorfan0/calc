...
function App() {
    ...
    function handleSndClick(e) {
        switch (ifNum) {
            case "not":
                equation === "" ? toast.error("Can't do this") : (setCalculations(equation + e), setEquation(""));
                break;
            default:
                if (calculations === "") {
    ...
    function handleClick(e) {
        if (calculations.slice(calculations.length - 1) === ")") {
            toast.error("Can't use a number straight after the closing bracket");
        } else {
            switch (e) {
                case ".":
                    if (dot === false) {
                        ...
                        toast.error("Can't use dot twice in the same number");
                    }
                    break;
                default:
                    setIfNum(ifNum + "num");
    ...
    function addBrackets() {
        empty ? (setCalculations(calculations + "("), setBrackets(1), setOpeningBrackets(openingBrackets + 1)) : toast.error("Can't do this");
    }

    function closeBrackets() {
        if (openingBrackets < closingBrackets + 1) {
            toast.error("Can't close brackets without opening them");
        } else {
    ...
    function minOrPlus() {
        if (numbs === "0" || numbs === "" || equation === "0" || nonNumbers.includes(calculations.slice(calculations.length - 1)) === true ||
            ...
            toast.error("Can only change signs on a number (different than zero)");
        } else {
            if (numbs === "" && equation !== "") {
                switch (sign) {
                    case "neg":
    ...
    return (
        <div className="App">
            ...
            <div className="cont">
                ...
                <Button clas="buttonNorm" text=":)" click={() => toast('🦄 :)', {
                    position: "top-center",
                    autoClose: 5000,
                    hideProgressBar: false,
                    closeOnClick: true,
